...
public class TBLModelCube 
{
	...
	public final TBLModelCube[] children;
	...
	private TBLModelCube(String name, Vector3f dimensions, Vector3f position,
			Vector3f offset, Vector3f rotation, Vector3f scale, int textureOffX, int textureOffY,
			boolean mirrorTX, TBLModelCube... children) 
	{
		this.name = name;
	...
	public void addChildren(ArrayList<TBLModelCube> list)
	{
		for(TBLModelCube cube : children)
		{
			list.add(cube);
	...
	public void globalizeTransform(boolean isChild)
	{
		...
		for(TBLModelCube child : children)
		{
	...
	static class Deserializer implements JsonDeserializer<TBLModelCube>
	{

		@Override
		public TBLModelCube deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
				throws JsonParseException 
		{
			JsonObject object = json.getAsJsonObject();
			return new TBLModelCube(
		...
		private TBLModelCube[] getChildren(JsonObject object, JsonDeserializationContext context)
		{
			if(!object.has("children"))
				return new TBLModelCube[0];
			
			JsonArray jsonarray = JsonUtils.getJsonArray(object, "children");
			TBLModelCube[] jaint = new TBLModelCube[jsonarray.size()];
			
			for (int i = 0; i < jaint.length; ++i)
            {
            	jaint[i] = context.deserialize(jsonarray.get(i), TBLModelCube.class);
            }
	...
	private TBLModelCube parent;
	
	public TBLModelCube getOriginalParent() {
		return parent == null ? this : parent.getOriginalParent();
	}
	
	public void setTjrmodel(TBLModelCube parent, TBLModel tjrmodel) 
	{
